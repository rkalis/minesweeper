...
function endgame:enter(previous, game, outcome)
    self.time = 0
    self.game = game
...
function endgame:update(dt)
    if self.time == 0 then
        love.audio.play(assets.audio[self.outcome])
        self.game.board:clear()
    end
    self.time = self.time + dt

    if self.outcome == "win" and self.time >= 3 then
...
function endgame:draw()
    ...
    self.game.ui.buttons.medium.smiley = self.outcome
